...
public class PedidoController {
    ...
    /**
     * NOVO MÉTODO: Filtra pedidos de um entregador específico.
     */
    public List<Pedido> listarPedidosPorEntregadorEStatus(Integer idEntregador, String status) {
        // 1. Busca TODOS os pedidos com aquele status
        List<Pedido> pedidosDoStatus = this.listarPedidosPorStatus(status);

        // 2. Cria uma lista nova só para os pedidos DESTE entregador
        List<Pedido> meusPedidos = new ArrayList<>();

        if (pedidosDoStatus != null) {
            for (Pedido p : pedidosDoStatus) {
                // Verifica se o pedido é deste entregador
                if (p.getEntregadorId() != null && p.getEntregadorId().equals(idEntregador)) {
                    meusPedidos.add(p);
                }
            }
        }
    ...
    /**
     * Busca todo o histórico de pedidos de um cliente específico.
     */
    public List<Pedido> listarPedidosPorCliente(int clienteId) {
        try {
            // Chama o DAO para buscar no banco
